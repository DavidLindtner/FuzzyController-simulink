function [incrementVoltage] = LargestOfMaxima(DOM,outputSet, memType)
...
%   All other rule
        ...
        switch memType
            case 'Triangullar'
                incrementVoltage = -(maxDom *(outputSet(maxDomIndex(1)+1) - outputSet(maxDomIndex(1))) - outputSet(maxDomIndex(1)+1));
            case 'Gaussian'
                incrementVoltage = halfBackGaussian(outputSet(maxDomIndex(1)), outputSet(maxDomIndex(1)+1), 'falling', maxDom);
        end
    end
end
